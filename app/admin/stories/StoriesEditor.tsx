...
export default function StoriesEditor({ initial, countries }: { initial: Story[]; countries: { id: string; ko: string }[] }) {
  ...
  function save() {
    setMsg(null);
    start(async () => {
      const res = await saveStories(list);
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <section className="admincard">
        ...
        <button className="abtn" style={{ marginTop: 14 }} onClick={add}>+ 이야기 추가</button>
      </section>
