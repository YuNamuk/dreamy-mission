...
export default async function StaticPage({ pageKey, active, children }: { pageKey: PageKey; active?: string; children?: React.ReactNode }) {
  const [user, settings, page] = await Promise.all([getUser(), getSettings(), getPage(pageKey)]);

  return (
    <main>
      <Nav user={user} countries={COUNTRIES.map((c) => ({ id: c.id, ko: c.ko, en: c.en }))} active={active} logo={settings.logoUrl} />
