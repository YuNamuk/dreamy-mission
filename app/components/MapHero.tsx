    ...
    (async () => {
      ...
      for (const c of countries) {
        ...
        const statusHtml = c.status && c.status.items.length
          ...
          : '';
        m.bindPopup(
          `<div class="mm-pop"><div class="mm-pop__hd"><b>${c.ko}</b><i>${c.en}</i></div><div class="mm-pop__sum">${c.summary}</div>${statusHtml}<a href="/mission/${c.id}" class="mm-pop__more" data-detail="${c.id}">${L_DETAIL}</a></div>`,
      ...
      // 좁은 화면에서는 좌우 여백을 넉넉히 둔다 — 표기가 핀보다 넓어서 34px 로도 잘렸다(390px 화면에서 실측)
      map.fitBounds(L.latLngBounds([SEOUL_LATLNG, ...dests]), { paddingTopLeft: [wide ? 300 : 78, wide ? 90 : 52], paddingBottomRight: [wide ? 320 : 78, wide ? 60 : 52] });
